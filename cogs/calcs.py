...
class Calculation(commands.Cog):
    ...
    async def distance(self, ctx, coord1, coord2):
        if coord1 == '':  # checking for blank arg
            await ctx.send(content="`***ERROR:*** You need to list the first system's coordinates.`")
        elif coord2 == '':  # checking for blank arg
            await ctx.send(content="`***ERROR:*** You need to list the second system's coordinates.`")
    ...
    async def commission(self, ctx, coord, mission, explored=True, ad=True, ppl=False, qs=False, w_stars=0, o_stars=0,
        ...
        if coord == '':  # checking for blank arg
            ...
            if mission == "Probe" or mission == "PB":
                ...
                await ctx.send("```Sending a Probe to {} will return a commission of {} Licenses.```"
                ...
                await ctx.send("```Sending Pioneers to {} will return a commission of {} Licenses.```"
                               .format(coord, commission))
                ...
                if qs:
                    ...
                    commission = 50
                await ctx.send("```Sending a Science Vessel to {} will return a commission of {} Licenses.```"
                               .format(coord, commission))
            else:
